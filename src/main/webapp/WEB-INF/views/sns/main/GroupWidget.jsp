...
var _GroupWidget_BASE_MEMBER      = '<%=mrf.BASE_MEMBER%>';
var _GroupWidget_session_memberId = '<%=session.getAttribute("memberId")%>';
var _GroupWidget_group_groupId    = '${group.groupId}';
var _GroupWidget_group_groupName  = '';
var gSMainType = "<%=sMainType%>";

$(function(){
	_GroupWidget_group_groupName  = $('#forSillyGroupName').val();
});
...
<c:choose>
  <c:when  test="${uiMode eq 'js'}">
	<script type="text/javascript" src="../jsx/build/sns/main/GroupWidget.js"></script>
  </c:when >
  <c:otherwise>
	<script type="text/jsx" src="../jsx/src/sns/main/GroupWidget.jsx"></script>
  </c:otherwise>
</c:choose>
...
<input type='hidden' id='forSillyGroupName' value='<c:out value="${group.groupName}" escapeXml="true" />' />
	<div class="lay-wrap">
	<c:choose>
