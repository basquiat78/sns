		...
		var CommentFeed = React.createClass({
			...
			getFollowerInfo: function(info) {
				...
				if(followerType ==='GROUP') {
					baseurl = _CommentFeed_BASE_FOLLOWER + '/group/'+followerId;
				}
			...
			modifyComment: function() {
				...
				var subtitleId   = "subtitle_"+this.state.feedId;
				React.render(<CommentFeedModifyElement contents={this.state.feedTitle} commentUpdate={this.commentUpdate} cancelModify={this.cancelModify} feedId={this.state.feedId}/>, document.getElementById(subtitleId));
			},
			...
			render: function() {
				...
				return (
						<div className="reply_contents" id={commentFeedId}>
							<dl style={{'marginLeft':'23px'}}>
	            				<dt ref='commentPic' style={{'float':'left','cursor':'pointer'}}><span className='presence-img'><img src={this.state.src} ref='ffImg' onMouseOver={this.openLyncMenuImg} onMouseOut={this.hideLyncMenu} style={imgStyle}></img></span><span className={presenceAreaClass} style={{'marginLeft':'-20px'}} id={presenceAreaId}></span></dt>
                				<dd className='feed_name' rel={tooltipInfo}><span rel={tooltipInfo} onClick={this.getMemberInfo} ref='ffName' onMouseOver={this.openLyncMenuName} onMouseOut={this.hideLyncMenu}>{this.state.cmtMember.memberName} {memberPosition}</span>&nbsp;&nbsp;<span style={{'color': '#27641b'}}>{targetMessege}</span><span ref='targetMember' onClick={this.getReceiveMemberInfo} onMouseOver={this.openLyncMenuReceiveName} onMouseOut={this.hideLyncMenu} rel={targetTooltipInfo} style={{'color': '#264f8c'}}>{targetName}</span></dd>
								<dd className='time_write'><abbr className='timeago' ref='timeago' title={timeago}>{titleTimeAgo}</abbr></dd>
                				<dd ref='subtitle' id={subtitleId} className='rco_txt' style={{'wordWrap':'break-word'}}>{this.state.feedTitle}</dd>
								<div id={commentSliderId} style={{'marginLeft':'55px', 'marginTop':'0', 'marginLeft':'35px'}}></div>
								<div className='af_list_wrap' style={{'marginLeft':'55px'}}>
									<div id={ogAreaId}></div>
								</div>
								<CommentAttachFileList fileList = {this.state.files}/>

            				</dl>

            				<ul className='rc_vote'>
								<li className='mark_vote' style={{'marginLeft':'0'}} onClick={this.cmtLikeit}><img src='../images/btn_like.png' width='14' height='13' /></li>
                                <li onClick={this.cmtLikeit}>{likeItByMeMsg}  </li>
                                <li id={'cmtfeedlikecnt' + this.state.feedId} onClick={this.cmtGetFanaticList} style={{'paddingLeft':'4px'}}><strong>{this.state.likeItCnt}</strong></li>
                                <li onClick={this.answerTag} className='mark_vote'><img src='../images/btn_reply.png' width='14' height='13' /></li>
                                <li onClick={this.answerTag}>{_FEED_CommentFeed_MSG_REPLY}</li>
                                <li onClick={this.share} className='mark_vote'><img src='../images/btn_sympathy.png' width='16' height='13' /></li>
                                <li onClick={this.share}>{_FEED_CommentFeed_MSG_SHARE}</li>
                                <li onClick={this.commentMoreLayout} style={{'display':'none'}}>{_FEED_CommentFeed_MSG_MORE}</li>
                                <li onClick={this.commentMoreLayout} style={{'marginLeft':'20px'}} className='mark_vote'><img src='../images/btn_more.png' width='18' height='13' /></li>
							</ul>
							<div id={commentLikeItByMeAreaId}></div>
							<div className='for_select2'>
								<div id={commentMoreLayoutId} className='more_select2' style={{'display':'none'}}>
                                   	<ul>
                                       	<li onClick={this.commentFollow} 		 ref='iconCommentFollowId'			className='icon_ms'><img src='../images/btn_selectmore1.png'/></li>
                                        <li onClick={this.commentFollow}		 ref='txtCommentFollowId'			className='txt_ms'>{_FEED_CommentFeed_MSG_FOLLOW}</li>
										<li onClick={this.commentUnfollow} 		 ref='iconCommentUnfollowId'		className='icon_ms'><img src='../images/btn_selectmore1.png'/></li>
                                        <li onClick={this.commentUnfollow}		 ref='txtCommentUnfollowId'			className='txt_ms'>{_FEED_CommentFeed_MSG_UNFOLLOW}</li>
                                        <li onClick={this.addCommentBookmark} 	 ref='iconAddCommentBookmarkId'		className='icon_ms'><img src='../images/btn_selectmore7.png'/></li>
                                        <li onClick={this.addCommentBookmark} 	 ref='txtAddCommentBookmarkId'		className='txt_ms'>{_FEED_CommentFeed_MSG_BOOKMARK}</li>
                                        <li onClick={this.removeCommentBookmark} ref='iconRemoveCommentBookmarkId' 	className='icon_ms'><img src='../images/btn_selectmore8.png'/></li>
                                        <li onClick={this.removeCommentBookmark} ref='txtRemoveCommentBookmarkId'	className='txt_ms'>{_FEED_CommentFeed_MSG_CLEARBOOKMARK}</li>  
                                        <li onClick={this.regCommentKldg} 		 ref='iconRegCommentKldgId'			className='icon_ms'><img src='../images/btn_selectmore2.png'/></li>
                                        <li onClick={this.regCommentKldg} 		 ref='txtRegCommentKldgId'			className='txt_ms'>{_FEED_CommentFeed_MSG_MAKEKNOWLEDGE}</li>
										<li onClick={this.removeCommentKldg} 	 ref='iconRemoveCommentKldgId'		className='icon_ms'><img src='../images/btn_selectmore2.png'/></li>
                                        <li onClick={this.removeCommentKldg} 	 ref='txtRemoveCommentKldgId'		className='txt_ms'>{_FEED_CommentFeed_MSG_CLEARKNOWLEDGE}</li>
										<li onClick={this.deleteComment} 		 ref='iconDeleteCommentId'			className='icon_ms'><img src='../images/btn_selectmore6.png'/></li>
                                        <li onClick={this.deleteComment} 		 ref='txtDeleteCommentId'			className='txt_ms'>{_FEED_CommentFeed_MSG_DELETE}</li>                       
                                   	</ul>
                                </div>
							</div>
						</div>
		...
		// Comment 파일
		var CommentAttachFileList = React.createClass({
			// 파일 리스트  렌더링
			render: function() {
				...
				if(data!==undefined && data.length>0) {
					data.forEach(function(file){
						if(file.fileId != 0) {
							filesArray.push(<CommentAttachFile key={file.fileId} file={file}/>);
						}
					});
					...
					if(filesArray.length > 0) {
						...
						return (<div style={{'borderBottom':'none'}}></div>);
					}
		...
		// 파일 
		var CommentAttachFile = React.createClass({
			// 파일  렌더링
			render: function() {
		...
		var CommentFeedModifyElement = React.createClass({
			...
			render: function() {
	        	return (
	        			<div className='modify_comment'>
							<div ref='modify_comment_area'>
                            	<textarea style={{'width':'99%', 'padding':'1% 4% 2% 2%', 'height':'32px', 'right':'10px', 'border':'none', 'resize':'none'}} className='modifyCommentContentsInput' id='modifyCommentContentsInput' ref='modifyCommentContentsInput'></textarea>
							</div>
							<div style={{'margin':'0px 35px 0 0px'}}>
								<button style={{'float':'right','cursor':'initial'}} type='button' id='commentUpdate' ref='commentUpdate' className='btn-m btn-m btn-attention' onClick={this.commentUpdate}>{_FEED_NormalType_UPDATEDBUTTON}</button>
								<button style={{'float':'right', 'marginRight':'5px', 'cursor':'initial'}} type='button' id='cancelModify' ref='cancelModify' className='btn-m btn-m btn-attention' onClick={this.cancelModify}>{_FEED_NormalType_CANCELBUTTON}</button>
								</div>
						</div>
	           	);
			}
		...
		function commentOGTrigger(reactElement, data) {
			...
			React.render(<CommentOG data={data} showRemove={showRemove}/>, document.getElementById(ogAreaId));
		}
		...
		function commentComponentDidMount(reactElement) {
			...
			if(reactElement.state.likeItByMe === 1) {
				React.render(<LikeIt />, document.getElementById(commentLikeItByMeAreaId));
			}
			...
			React.render(<Slider fileList = {reactElement.state.files} feedtype={'commentFeed_'} feedId={reactElement.state.feedId}/>, document.getElementById(commentSliderId));
		...
		function commentShare(reactElement) {
			React.render(<SharePopup shareData={reactElement.shareFollower} shareMemberId={_CommentFeed_session_memberId} shareFeedId={reactElement.state.pFeedId}/>, document.getElementById('share_popup'));
		...
		function commentAnswerTag(reactElement) {
			...
			React.render( <Answer receiveMember={reactElement.state.cmtMember} removeAnswerTag={reactElement.removeAnswerTag}/>, document.getElementById(answerTagId) );
