		// 팔로워 툴팁
		var TooltipGroup = React.createClass({
			...
			render: function() {
				...
				if(this.props.lastComma ==='true' && this.props.extra === 0 ) {
					return (
							<span className='link_gr' onClick={this.goGroup}>{this.props.group.groupName}, </span>
					);
					...
					return (
							<span>{_FEED_Follower_MSG_TOOLTIPTEXT1} {this.props.extra}{_FEED_Follower_MSG_TOOLTIPTEXT2}</span>
					);
				}
			}
		...
		// 팔로워 툴팁
		var FollowerTooltip = React.createClass({
	...
	*/			
			...
			render: function() {
				...
				if(this.props.followerInfo.groupType == '0' || this.props.followerInfo.groupType === 'ORGANIZATION') {
					...
					return (
							<div className='over_feed_gate'>
        	                	<dl>
            	                	<dt className='pic_group_big' ref='followerPic'>&nbsp;</dt>	
                	                <dd className='malgun16'><span className='tooltip_link' onClick={this.goGroupContents}>{this.props.followerInfo.groupName}</span></dd>
                    	            <dd className='malgun12'>{this.props.followerInfo.description}&nbsp;</dd>
                        	        <dd className='malgun12'>{_FEED_Follower_MSG_FTT_FOLLOWERTEXT}({this.props.followerInfo.memberCnt})</dd>                           
                            	</dl> 
                            	<span className='send_mail_wrap' style={{'cursor':'initial'}}>
									<button style={{'cursor':'initial','visibility':'hidden'}} type='button' ref='postup' className='btn-m btn-default'>{_FEED_Follower_MSG_FTT_SENDMSGBTNTEXT}</button>
								</span>                                
                        	</div>
						);
				} else {
					...
					if(groupListLength>3) {
						...
						for(var i=0; i<3; i++) {
							...
							tooltipGroupList.push(<TooltipGroup key={key} lastComma={lastComma} extra={extra} group={group}/>)
						}

						key = 'tooltipExtra_'+itemId;
						lastComma = 'false';
						extra = groupListLength - 3;
						tooltipGroupList.push(<TooltipGroup key={key} lastComma={lastComma} extra={extra}/>)
					...
					return (
							<div className='over_feed_gate'>
        	       	        	<dl>
            	   	            	<dt className='pic_big' ref='followerPic' onClick={this.getMemberInfo} style={{'cursor':'pointer'}}>&nbsp;</dt>	
                	                <dd className='malgun16' onClick={this.getMemberInfo}><span className='tooltip_link'>{this.props.followerInfo.followerName} {positionName}</span></dd>
                   		            <dd className='malgun12'>{_FEED_Follower_MSG_FTT_GROUPTEXT}:{tooltipGroupList}&nbsp;</dd>
                   	    	        <dd className='malgun12'>{_FEED_Follower_MSG_FTT_TEAMTEXT}: {partName}&nbsp;</dd>
									<span className='send_mail_wrap' style={{'cursor':'initial'}}>
										<button style={{'cursor':'initial','visibility':'hidden'}} type='button' ref='postup' className='btn-m btn-default' onClick={this.writeFeed}>{_FEED_Follower_MSG_FTT_SENDMSGBTNTEXT}</button>
									</span>
                   	        	</dl> 
                   	    	</div>
						);
		...
		// 팔로워 참조 
		var FeedFollowerList = React.createClass({
			// 팔로워 참조  렌더링
			render: function() {
				...
				if(data.length>0) {
					data.forEach(function(feedFollower){
						if(feedFollower.followerType === 'GROUP') {
							...
							ffArray.push(<FeedFollower key = {key} feedFollower={feedFollower} type={'GROUP'}/>);
						}
					});
					...
					data.forEach(function(feedFollower){
						if(feedFollower.followerType === 'MEMBER') {
							...
							ffArray.push(<FeedFollower key = {key} feedFollower={feedFollower} type={'MEMBER'}/>);
						}
					});
					...
					if(exceptionCnt > 0) {
						ffArray.push(<FeedFollower key = {'exeption'} followerCnt={exceptionCnt} feedFollower={[]} type={'EXE'}/>);
					}
		
					return (
							<span className="call_target reply-layout">
           	  					<ul>
		            	        	<li style={{'textDecoration':'none'}}>{_FEED_Follower_MSG_REFERENCETEXT} :</li>
									{ffArray}
	            	 			</ul>
	            			</span>  
							);
		...
		// 팔로워 참조 
		var FeedFollower = React.createClass({
			...
			tooltipRender: function(data) {
				React.render(<FollowerTooltip followerInfo={data} itemId={this.props.feedFollower.itemId}/>, document.getElementById('sns-tooltip'));
			},
			...
			goGroup: function() {
				...
				if(contentsType == 'USER') {
					React.unmountComponentAtNode(document.getElementById('RightUpLevel'));
					React.render(<MyGroupFollower groupId={this.props.feedFollower.followerId}/>, document.getElementById('RightUpLevel'));
				}
				...
				// 가운데 컨텐츠 상단 부분(그룹 정보)
				React.unmountComponentAtNode(document.getElementById('head_contents'));
				React.render(<GroupHead groupId={this.props.feedFollower.followerId} />, document.getElementById('head_contents'));
				...
				// 중하단 탭 리스트 및 피드
				React.unmountComponentAtNode(document.getElementById('selectTabBySession'));
				React.render(<GroupTabList groupId={this.props.feedFollower.followerId} />,  document.getElementById('selectTabBySession'));
			...
			render: function() {
				...
				if(this.props.type == 'GROUP') {
					return (
           	    	    	<li ref='popupTrigger' rel={rel} onClick={this.getMemberInfo}>{this.props.feedFollower.followerName}</li>
							);

				} else if(this.props.type == 'MEMBER') {
					return (
           	    	    	<li ref='ffName' onClick={this.getMemberInfo} onMouseOver={this.openLyncMenuName} onMouseOut={this.hideLyncMenu}>{this.props.feedFollower.followerName}</li>
							);
				} else if(this.props.type == 'EXE') {
					return (
           	    	    	<li style={{'textDecoration':'none'}}>외 {this.props.followerCnt}명</li>
							);
				}
			}
		...
		var FollowerList = React.createClass({
			...
			render: function() {
				...
				this.props.fList.forEach(function(follower) {
					if(follower.type === 'GROUP' || follower.type === 'ORGANIZATION') {
						var key = 'follower'+follower.type+follower.id;
    	   				followers.push(<Follower key={key} follower={follower} removeEvent={that.removeHandler}/>);
					}
        		});
				...
				return (
						<dl>
							{followers}
							<FollowerInput fList={this.props.fList} 
		...
		var FollowerInput = React.createClass({
			...
			render: function() {
				...
				return(
						<dd className='none_attribute'>
							<input type='text' placeholder={_FEED_Follower_MSG_GROUPORFOLLOWERADDTEXT} className='follower_add_input ignore' ref='follower'/>
						</dd>
				);
			}
				...
				return(
						<dd className='lock_group'>
							<span><img src={src} width='9' height='11' style={style}/></span>
						   	<span>{this.props.follower.label}</span>
							<span className='space_left'><img src='../images/icon_delete.png' width='8' height='8' onClick={this.removed} style={{'marginRight':'3px'}}/></span>
						</dd>
					);
			}
		...
		var ShareFollowerApp = React.createClass({
			...
			render: function() {
				...
				return (
	        		   	<div className='notice_target'>
							<FollowerList writtenfeedFollower={this.props.writtenfeedFollower} dropFollower={this.state.dropFollower} fList={this.state.followerList} removeFollower={this.removeFollower} selectFollower={this.selectFollower}/>
	        	     	</div>
		            );
	        }
		});
		...
		var FollowerApp = React.createClass({
			...
			render: function() {
				...
				if(this.props.ftype == 'normal'){
					return (
	        	    	<div className='notice_target' onClick={this.focuson}>
							<FollowerList writtenfeedFollower={this.props.writtenfeedFollower} 
					...
					return (
	        	    		<div className='notice_target mainDrop-area' ref='dropZone' onClick={this.focuson}>
								<ul className='select_all'>
	                        		<li className='all_target'><input ref='companyAll' id='companyAll' type='checkbox' value='N' onClick={this.checkBox} /></li>
	                        		<li className='all_target_name'><label htmlFor='companyAll'>{_FEED_Follower_MSG_ALLCOMPANY}</label></li>                    
	                    		</ul>
								<FollowerList writtenfeedFollower={this.state.followerList} 
											  fList={this.state.followerList} 
											  removeFollower={this.removeFollower} 
											  selectFollower={this.selectFollower}
											  dropFollower={this.state.dropFollower}
		...
		var CommentFollowerInput = React.createClass({
			...
			render: function() {
					...
					return(
							<dd className='none_attribute'>
								<input type='text' placeholder={_FEED_Follower_MSG_REPORTOTHERPEOPLE} className='ignore' id={inputObjectid} ref='follower'/>
							</dd>
					);
			}
		...
		var CommentFollowerList = React.createClass({
				...
				followerList.forEach(function(follower) {
					if(follower.type === 'MEMBER') {
						var key = 'comment'+follower.type+follower.id;
           				followers.push(<CommentFollower key={key} follower={follower} removeEvent={that.removeHandler}/>);
					}
       			});

				return(
						<div className='tag_area' style={{'padding':'0'}}>
							<dl style={{'paddingTop':'0', 'paddingBottom':'3px', 'left':'0', 'right':'0', 'marginLeft':'9px'}} onClick={this.focuson}>
								{followers}
								<CommentFollowerInput 
		...
		var CommentFollower  = React.createClass({
			...
			render: function() {
				...
				return (
						<dd className='lock_group'>
							<span><img src={src} width='9' height='11' style={style}/></span>
							{label}
							<span className='space_left'>
								<img src='../images/icon_delete.png' width='8' height='8' onClick={this.removed}/>
							</span>
						</dd>
					);
			}
		...
		var CommentFollowerApp = React.createClass({
