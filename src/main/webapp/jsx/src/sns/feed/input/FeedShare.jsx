		var SharePopup = React.createClass({
			...
			getFeedDetailResult: function(feeddata){
				...
				if(document.getElementById('MemberFeedBox')) {
					...
					React.render(<FeedBox baseurl={_FeedShare_BASE_FEED} memberId={member.memberId} groupId={0}/>, document.getElementById('MemberFeedBox'));
			...
			render: function() {
	        	return (
						<div>
							...
							<div className="pop-modalwindow-header" style={{'cursor':'move'}}>
								<div className="pop-modalwindow-title">{_FEED_FeedShare_MSG_SHAREBTNTEXT}</div>
						        <div className="pop-modalwindow-header-option">
						        	<a className="ico-anchor"><span className="ico-img, ico-close" onClick={this.shareClosePopup}>{_FEED_FeedShare_MSG_CLOSEBTNTEXT}</span></a>
						    	</div>
							</div>
       						<div className='share_area_wrap' style={{'overflowX':'hidden','overflowY':'auto'}}>
           						<div id='ffList'>
									<ShareFeedDropzoneComponent config={componentConfig} 
           						...
           						<div className='share_feedcontents' style={{'height':'150px','border':'0'}}>
               						<dl>
	                   					<dt className='pic_small' ref='sharePic'></dt>
                   						<dd className='feed_name'><div id='shareUserName'></div></dd>
               						</dl>
               						<div className='substance' id='sfeedTitle'></div>   
           						</div> 
			       			</div>
