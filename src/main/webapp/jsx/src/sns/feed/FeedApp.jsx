		var FeedApp = React.createClass({
			...
			resetFeed: function(data) {
				...
				if($('#recentAct').length == 1 && $('#myTodoList').length == 1) {
					...
					React.render(<RecentActList baseurl={_FeedApp_MEMBER_WIDGET_ACTIVITY}/>, document.getElementById('recentAct'));
			...
			OGTrigger: function(data) {
				var showRemove = 'true';
				React.render(<MainOG data={data} removeOG={this.removeOGTag} showRemove={showRemove}/>, document.getElementById('ogArea'));
				addDrawTopLine();
			},
			...
			render: function() {
        		...
        		return ( <MainFeedDropzoneComponent 
