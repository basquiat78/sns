		...
		var MainContents = React.createClass({
			componentDidMount: function() {
				...
				if(this.props.mainType =='main') {
					// 개인, 그룹 정보 나오는 창
					React.render(<MemberHead memberId={_MainContents_targetMemberId}/>, document.getElementById('head_contents'));
					...
					if(_MainContents_session_memberId == _MainContents_targetMemberId){
						React.render(<MemberTabList memberId={_MainContents_session_memberId} detailFeedId={this.props.detailFeedId}/>,  document.getElementById('selectTabBySession'));
					}else{
						React.render(<OtherMemberTabList memberId={_MainContents_targetMemberId}/>,  document.getElementById('selectTabBySession'));
					}
				} else if(this.props.mainType =='group') {
					React.render(<GroupHead groupId={this.props.groupInfo.groupId} />, document.getElementById('head_contents'));
					// 가운데 컨텐츠 상단 부분(그룹 정보)
					React.render(<FeedApp groupInfo={this.props.groupInfo}/>, document.getElementById('mainContentsArea'));

					React.render(<GroupTabList groupId={this.props.groupInfo.groupId}/>,  document.getElementById('selectTabBySession'));
				} else {
...
//				this.integrationSearch();
			...
			render: function() {
				...
				return (
					<div className='feed_wrap'>
						<dl className='home_feed_gate'>
    						<dt>{_memberCompanyName}</dt>
        					<dd className='find_box'>
        						<ul>
        							<li id="itgBack" style={{'display':'none'}} onClick={this.goItgBack}><button className="btn-m btn-attention">back</button></li>
        							<li id="toTalNotiBack" style={{'display':'none'}} onClick={this.goTotalNotiBack}><button className="btn-m btn-attention">back</button></li>
            						<li id="FavoriteListBack" style={{'display':'none'}} onClick={this.goFavoriteListBack}>
            							<input type="hidden" id="myCurrentTodoIdInputHidden" />
            							<input type="hidden" id="myCurrentFavoriteIdInputHidden" />
            							<button className="btn-m btn-attention">back</button>
            						</li>
            						<li><input id='pItgSch' name='pItgSch' type='text' placeholder='Find' style={{'borderRight':'0'}} /></li>
                					<li className='btn_find' onClick={this.integrationSearch}></li>
            					</ul>
        					</dd>
    					</dl>
						
						<div id='feed_notice'></div>
						
						<div id="feedGate" className='feed_gate'>
							<div id='head_contents' style={{'overflow':'hidden','height':'90px'}}></div>
							<div id='mainContentsArea'></div>
						</div>
						
						<div className='search_tabstyle' id='selectTabBySession' style={{'marginBottom':'10px'}}></div>
						<div className='search_tabstyle' id='selectTabByFavorite' style={{'display':'none'}}></div>
						<div className='search_tabstyle' id='selectTabByIntegration' style={{'display':'none'}}></div>
						<div className='search_tabstyle' id='selectTabBySystem' style={{'display':'none', 'borderBottom': 'none'}}></div>
						<div className='search_tabstyle' id='selectTabByNoti' style={{'display':'none'}}></div>
		...
		function goTotalNotiList(aldShowYn, o) {
			...
			React.render(<TotalNotiTab g={o.g}/>, document.getElementById('selectTabByNoti'));
...
//			React.render(<HomeSNS mainType={'main'}/>, document.getElementById('WholeScreen'));
		...
		function openIntegrationBody(txt){
			...
			React.unmountComponentAtNode(document.getElementById('selectTabByIntegration'));
			React.render(<IntegrationTab/>, document.getElementById('selectTabByIntegration'));
