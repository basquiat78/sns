function reRenderingNewGroupList() {
	...
	//신규 그룹
	var NewGroupOne = React.createClass({
		...
		render : function() {
			...
			return (
				<dl className='new_group'>
                    ...
                    <dd className="per_email line_space" style={{'color':'#999999'}}>{regdate}<span style={{'marginLeft':'17px'}}>Member <strong className="txt_point" style={{'color':'#333', 'cursor':'pointer'}}>{this.state.memberCnt}</strong></span></dd>
                </dl>
			);
		}
	});
	
	var NewGroupList = React.createClass({
		...
		render : function() {
			var newGroupNodes = this.props.data.map(function (group, index) {
				return (
					<NewGroupOne key={index} groupId={group.groupId} groupName={group.groupName} groupImgUrl={group.groupImgUrl} regMemberName={group.regMemberName}
