	...
	function renderSideNavigator(data, type, option) {
		...
		React.render(React.createElement(RecentActList, {currGroupId: _GroupWidget_group_groupId, baseurl: contextpath + _SideNav_GROUP_WIDGET_ACTIVITY}), document.getElementById('recentAct'));
	...
	function renderRightLevel(type, option) {
		if(option !== undefined && option == 'init') {
			...
			if(contentsType == 'USER') {
				...
				React.render(React.createElement(RightContentArea, {mainType: type, groupId: _GroupWidget_group_groupId}), document.getElementById('RightContentArea'));
			}
			...
			switch (num) {
  				...
  				case 2 :
  						renderRightLevel(type, option); 
               			break;
  				case 3 : 
  						React.render(React.createElement(MainContents, {mainType: type, detailFeedId: '0', groupInfo: group}), document.getElementById('MainContents'));
               			break;
			}		
		});
