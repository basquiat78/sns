var TotalNotiTab  = React.createClass({displayName: "TotalNotiTab",
	...
	TotalNotiFeedRender : function(data) {
		...
		if( typeof data == "undefined" || data.length == 0) {
			...
			React.render(React.createElement(TotalNotiElemList, {key: 'justice', data: data}), document.getElementById(curMoreDivId));
		}
		...
		return (
				React.createElement("div", {className: "feed_wrap"}, 
					React.createElement("div", {id: "TotalNotiTabList"}, 
						React.createElement("ul", {id: "totalnoti_malgun", className: "malgun13", style: {'marginBottom':'20px'}}, 
							React.createElement("li", {className: "tab_on", style: {'color':'#fe630f', 'fontWeight':'bold'}}, _TotalNotiTabList_MSG_TOTALNOTITITLE)
						), 
...
var TotalNotiElem = React.createClass({displayName: "TotalNotiElem",
	...
	render : function() {
		...
		return (
			React.createElement("tr", {id: tnData.noticeId, key: key, style: {'height':'3em'}, dangerouslySetInnerHTML: {__html: trStr}}
...
var TotalNotiElemList = React.createClass({displayName: "TotalNotiElemList",
	...
	render: function() {
		...
		if(this.props.data.length > 0){
			FeedTotalNotiElemNodes = this.props.data.map(
        		function (feeddata, index) {
					...
					return (
						React.createElement(TotalNotiElem, {
							tnData: feeddata}
						)
					);
    	...
    	return (React.createElement("div", null, FeedTotalNotiElemNodes));
	}
});
