		...
		var NoFeedHere = React.createClass({displayName: "NoFeedHere",
			render : function() {
				return (
		...
		var NoticeInFeed = React.createClass({displayName: "NoticeInFeed",
					render: function() {
			        	return (
								React.createElement("div", null, 
		                        	React.createElement("div", {className: "noticeTitle"}, React.createElement("strong", null, "[", _Feed_basic_notiheader, "]"), " ", this.props.feedTitle, " ")
		                        )    
			           	);
					}
		...
		// 첨부 파일
		var LikeIt = React.createClass({displayName: "LikeIt",
			// 파일 리스트  렌더링
			render: function() {

					return (
							React.createElement("div", null, 
								React.createElement("ul", {className: "result_wrap2", style: {'marginLeft':'0px'}}, 
                              		React.createElement("li", {className: "result_like"}, React.createElement("img", {src: "../images/btn_like.png", width: "14", height: "13"})), 
                                	React.createElement("li", null, _FEED_Feed_MSG_ILIKETHISONEMSG)
                              	)
            				)  
					);
			}
		...
		// 전자결재 코멘트
		var ApprovalComment = React.createClass({displayName: "ApprovalComment",
			...
			render: function() {
					...
					approvaButtonSettings.push(React.createElement(ApprovalButton, {completeAppr: this.props.completeAppr, taskId: this.props.taskId, key: key, feedId: this.props.feedId}));

					return (
							React.createElement("div", {className: "my_comment", style: {'paddingTop':'0'}}, 
								React.createElement("dl", null, 
									approvaButtonSettings, 
                					React.createElement("dd", {className: "pic_small", ref: "memberPic"}, "pic"), 
									React.createElement("dd", {className: "feed_name"}, _FEED_Feed_MSG_APPROVALOPINION), 
                					React.createElement("dd", null, 
										React.createElement("textarea", {id: commentId, className: "apprCommentArea", placeholder: _FEED_Feed_MSG_INPUTOPINIONMSG, ref: "apprCommentArea"})
									)
           						)
							)
					);
		...
		// 전자결재 버튼
		var ApprovalButton = React.createClass({displayName: "ApprovalButton",
			...
			render: function() {
					...
					return (
							React.createElement("div", {className: "approval"}, 
								React.createElement("ul", {className: "btn_request_area"}, 
                            		React.createElement("li", {className: "btn_return", onClick: this.reject}, _FEED_Feed_MSG_RETURN), 
                            	    React.createElement("li", {className: "btn_approve", onClick: this.approval}, _FEED_Feed_MSG_APPROVAL), 
									React.createElement("li", {className: "apprPassword"}, React.createElement("input", {placeholder: "Password", type: "password", ref: "apprPassword"}))
                            	)
							)  
					);
			}
		...
		// 첨부 파일
		var FeedAttachFileList = React.createClass({displayName: "FeedAttachFileList",
			// 파일 리스트  렌더링
			render: function() {
				...
				if(data!==undefined && data.length>0) {
					data.forEach(function(file){
						if(file.fileId != 0) {
							filesArray.push(React.createElement(FeedAttachFile, {key: file.fileId, file: file}));
						}
					});
		...
		// 파일 
		var FeedAttachFile = React.createClass({displayName: "FeedAttachFile",
			// 파일  렌더링
			render: function() {
				...
				return (
						React.createElement("div", {className: "af_list01"}, 
                    		React.createElement("dl", null, 
		...
		var CommentNoti = React.createClass({displayName: "CommentNoti",
			...
			render: function() {
					...
					return (
							React.createElement("div", {onClick: this.newCommentShow}, 
								React.createElement("ul", {className: "result_wrap", style: {'marginLeft':'30px', 'marginRight':'20px', 'fontWeight': 'bold', 'cursor':'pointer'}}, 
                	                	React.createElement("li", null, this.props.noticeCnt, _FEED_Feed_MSG_NEWREPLYMSG, " ›››")
                	           	)
           					)  
					);
			}
		...
		var Feed = React.createClass({displayName: "Feed",
			...
			feedModify: function() {
				...
				React.render(React.createElement(FeedModifyElement, {contents: this.state.feedTitle, feedUpdate: this.feedUpdate, cancelModify: this.cancelModify, feedId: this.state.feedId}), document.getElementById(titleId));
			},
			...
			render: function() {
				...
				if(this.props.commentFeedList != null){
					commentRealNodes = this.props.commentFeedList.map(
						function (ctl) {
							var key = 'feedId'+ctl.feedId;
							return (
									React.createElement(CommentFeed, {key: key, 
												 commentData: ctl, 
											 	 pFeedData: self.state, 
				...
				if(this.state.feedType =='POLL' && this.state.isVote == 'false') {
					...
					pollNode.push(React.createElement(FeedPollView, {key: key, 
					...
					resultPollNode.push(React.createElement(FeedPollResultView, {key: key, 
													  votePoll: this.votePoll, 
													  feedId: this.state.feedId, 
													  feedTitle: this.state.feedTitle, 
													  pollList: this.state.feedPollList, 
				...
				if(this.state.feedType ==='NOTICE') {
					...
					try {
						...
						noticeTitle.push(React.createElement(NoticeInFeed, {key: noticeKey, feedTitle: this.state.feedTitle, data: noticeJsonData}));
				...
				if(bookMarkByMeChecker == 'true') {
					reactBookMarkHtmlElement = 
						React.createElement("dd", {className: "icon_favorite", ref: "favoriteIconId", onClick: this.removeFeedBookmark, title: _FEED_Feed_MSG_BOOKMARKHOVERTITLE})
				} else { // 북마크아니라면
					reactBookMarkHtmlElement = 
						React.createElement("dd", {className: "icon_favorite2", ref: "favoriteIconId", onClick: this.addFeedBookmark, title: _FEED_Feed_MSG_BOOKMARKHOVERTITLE})
				}
				...
				if(publicFeedChecker == 0) {
					reactPublicFeedHtmlElement = 
						React.createElement("dd", {className: "icon_lock", title: _FEED_Feed_MSG_SECRETHOVERTITLE})
				} else { // 공개그룹의 피드라면
					reactPublicFeedHtmlElement = 
						React.createElement("dd", null)
				}
	        	...
	        	return (
	        		React.createElement("div", {id: feedAreaId}, 
						React.createElement(FeedInGroupFeed, {feedId: this.state.feedId, feedfollower: this.state.feedfollower, groupId: this.state.groupId}), 
						React.createElement("div", {onMouseOver: this.focusFeedArea, ref: "focusDIV", className: "feed_contents"}, 
			        		React.createElement("div", {ref: "feed_contents", style: {'borderTop':'none'}}, 
		    	    			React.createElement("dl", null, 
									React.createElement("dt", {ref: "memberPic", rel: tooltipInfo, style: {'float':'left','cursor':'pointer'}}, React.createElement("span", {className: "presence-img"}, React.createElement("img", {src: this.state.src, ref: "ffImg", onMouseOver: this.openLyncMenuImg, onMouseOut: this.hideLyncMenu, style: imgStyle})), React.createElement("span", {className: presenceAreaClass, style: {'marginLeft':'-20px'}, id: presenceAreaId})), 
									React.createElement("dd", {className: "feed_name", rel: tooltipInfo}, React.createElement("span", {rel: tooltipInfo, onClick: this.getMemberInfo, ref: "ffName", onMouseOver: this.openLyncMenuName, onMouseOut: this.hideLyncMenu}, this.state.regMember.memberName, " ", positionName), React.createElement("span", {className: "belong_people"}, partName)), 
									...
									React.createElement("dd", {className: knowledgeClass, ref: "knowledgeId", title: _FEED_Feed_MSG_KNOWLEDGEHOVERTITLE}), 
									reactBookMarkHtmlElement, 
                					reactPublicFeedHtmlElement
		            			), 

								React.createElement("div", {className: "substance reply-layout", ref: "substance", style: {'whiteSpace':'pre-line'}}, 
									React.createElement("strong", {id: duedateAreaId}), 
		            				React.createElement("div", {ref: "feedTitle", id: titleId}, feedTitle)
            					), 
								
								React.createElement("div", {id: sliderId, className: "reply-layout", style: {'clear':'both'}}), 
								React.createElement("div", {ref: "ffList"}, 
									React.createElement(FeedFollowerList, {feedfollower: orgFeedFollower, followerCnt: this.props.followerCnt})
								), 
								React.createElement("div", {className: "af_list_wrap reply-layout"}, 

									React.createElement("div", {id: ogAreaId, ref: "ogArea"}), 

									React.createElement(FeedAttachFileList, {fileList: this.props.files})

								), 

								React.createElement("ul", {className: "ss_vote reply-layout", ref: "voteArea"}, 
									React.createElement("li", {onClick: this.feedLikeit, className: "mark_vote", style: {'marginLeft':'0'}}, React.createElement("img", {src: "../images/btn_like.png", width: "14", height: "13"})), 
                                    React.createElement("li", {onClick: this.feedLikeit}, likeItByMeMsg), 
                                    React.createElement("li", {id: 'feedlikecnt' + this.state.feedId, onClick: this.getFanaticList, style: {'paddingLeft':'4px'}}, React.createElement("strong", null, this.state.likeItCnt)), 
                                    ...
                                    React.createElement("li", {onClick: this.answerTag}, _FEED_Feed_MSG_COMMENTTEXT, " ", React.createElement("strong", null, this.state.cmtCnt)), 
                                    React.createElement("li", {className: "mark_vote", onClick: this.share}, React.createElement("img", {src: "../images/btn_sympathy.png", width: "16", height: "13"})), 
                                    React.createElement("li", {onClick: this.share}, _FEED_Feed_MSG_SHARETEXT, " ", React.createElement("strong", null, this.state.shareCnt)), 
                                    React.createElement("li", {onClick: this.feedMoreLayout, style: {'marginLeft':'20px','display':'none'}}, _FEED_Feed_MSG_MORETEXT), 
                                    React.createElement("li", {onClick: this.feedMoreLayout, style: {'marginLeft':'20px'}, ref: "moreDiv", className: "mark_vote2"}, React.createElement("img", {src: "../images/btn_more.png", width: "18", height: "13"})), 	

									React.createElement("div", {id: feedMoreLayoutId, className: "more_select", style: {'display':'none'}}, 
                                    	React.createElement("ul", null, 
                                        	React.createElement("li", {onClick: this.feedFollow, 			className: "icon_ms", ref: "iconFollowId"}, React.createElement("img", {src: "../images/btn_selectmore1.png"})), 
                                            React.createElement("li", {onClick: this.feedFollow, 			className: "txt_ms", ref: "txtFollowId"}, _FEED_Feed_MSG_LAYOUTFOLLOW), 
											React.createElement("li", {onClick: this.feedUnfollow, 		className: "icon_ms", ref: "iconUnfollowId"}, React.createElement("img", {src: "../images/btn_selectmore1.png"})), 
                                            React.createElement("li", {onClick: this.feedUnfollow, 			className: "txt_ms", ref: "txtUnfollowId"}, _FEED_Feed_MSG_LAYOUTUNFOLLOW), 
                                            React.createElement("li", {onClick: this.addFeedBookmark, 		className: "icon_ms", ref: "iconAddBookmarkId"}, React.createElement("img", {src: "../images/btn_selectmore7.png"})), 
                                            React.createElement("li", {onClick: this.addFeedBookmark, 		className: "txt_ms", ref: "txtAddBookmarkId"}, _FEED_Feed_MSG_LAYOUTBOOKMARK), 
                                            React.createElement("li", {onClick: this.removeFeedBookmark, 	className: "icon_ms", ref: "iconRemoveBookmarkId"}, React.createElement("img", {src: "../images/btn_selectmore8.png"})), 
                                            React.createElement("li", {onClick: this.removeFeedBookmark, 	className: "txt_ms", ref: "txtRemoveBookmarkId"}, _FEED_Feed_MSG_LAYOUTCLEARBOOKMARK), 
                                            React.createElement("li", {onClick: this.regFeedKldg, 			className: "icon_ms", ref: "iconRegFeedKldgId"}, React.createElement("img", {src: "../images/btn_selectmore2.png"})), 
                                            React.createElement("li", {onClick: this.regFeedKldg, 			className: "txt_ms", ref: "txtRegFeedKldgId"}, _FEED_Feed_MSG_LAYOUTKNOWLEDGE), 
                                           	React.createElement("li", {onClick: this.removeFeedKldg, 		className: "icon_ms", ref: "iconRemoveFeedKldgId"}, React.createElement("img", {src: "../images/btn_selectmore2.png"})), 
                                            React.createElement("li", {onClick: this.removeFeedKldg, 		className: "txt_ms", ref: "txtRemoveFeedKldgId"}, _FEED_Feed_MSG_LAYOUTCLEARKNOWLEDGE), 
                                            React.createElement("li", {onClick: this.makeGroup, 			className: "icon_ms", ref: "iconMakeGroupId"}, React.createElement("img", {src: "../images/btn_selectmore2.png"})), 
                                            React.createElement("li", {onClick: this.makeGroup, 			className: "txt_ms", ref: "txtMakeGroupId"}, "그룹만들기"), 
											React.createElement("li", {onClick: this.feedComplete, 		className: "icon_ms", ref: "iconFeedCompleteId"}, React.createElement("img", {src: "../images/btn_selectmore3.png"})), 
                                            React.createElement("li", {onClick: this.feedComplete, 		className: "txt_ms", ref: "txtFeedCompleteId"}, _FEED_Feed_MSG_LAYOUTCOMPLETE), 
                                            React.createElement("li", {onClick: this.feedIncomplete, 		className: "icon_ms", ref: "iconFeedIncompleteId"}, React.createElement("img", {src: "../images/btn_selectmore4.png"})), 
                                            React.createElement("li", {onClick: this.feedIncomplete, 		className: "txt_ms", ref: "txtFeedIncompleteId"}, _FEED_Feed_MSG_LAYOUTINCOMPLETE), 
                                            React.createElement("li", {className: "icon_ms", ref: "iconChangFeedDateId"}, React.createElement("img", {src: "../images/btn_selectmore5.png"})), 
                                            React.createElement("li", {className: "txt_ms", ref: "txtChangFeedDateId"}, React.createElement("input", {type: "hidden", ref: "inputChangFeedDateId"}), _FEED_Feed_MSG_LAYOUTCHANGEDATE), 
                                            React.createElement("li", {onClick: this.deleteFeed, 			className: "icon_ms", ref: "iconDeleteFeedId"}, React.createElement("img", {src: "../images/btn_selectmore6.png"})), 
                                            React.createElement("li", {onClick: this.deleteFeed, 			className: "txt_ms", ref: "txtDeleteFeedId"}, _FEED_Feed_MSG_LAYOUTDELETE)
                                    	)
                                    )	
							...
							React.createElement(ResultTagList, {
							...
							React.createElement("div", {className: "reply-layout", ref: "likeitArea", id: feedLikeItByMeAreaId, style: {'marginLeft':'0px','marginRight':'0px'}}), 							
							React.createElement("div", {id: apprCommentId, className: "apprCommentDiv reply-layout"}), 
							React.createElement("div", {className: "reply-layout"}, 
								React.createElement("div", {id: moreReplyId, style: showMoreBtnStyle, className: noview_reple, onClick: this.getBeforeCommentList}, this.state.commentListBefore), 
								this.state.commentListReal
							), 		
							React.createElement("div", {className: "reply-layout", id: notiMsgId}), 
							React.createElement(CommentDropzoneComponent, {
		...
		var FeedList = React.createClass({displayName: "FeedList",
			...
			render: function() {
	        	...
	        	return (React.createElement("div", {ref: "feedList"}, 
	        				React.createElement("div", {ref: "element"}, feedNodes)
	        			));
		...
		var FeedInGroupFeed = React.createClass({displayName: "FeedInGroupFeed",
			getInitialState: function() {
			...
			tooltipRender: function(data) {
				React.render(React.createElement(FollowerTooltip, {followerInfo: data, itemId: this.props.feedId}), document.getElementById('sns-tooltip'));
			},
			...
			render: function() {
				...
				if(followerName != '' && this.props.groupId === 0){
	        		...
	        		return (
							React.createElement("ul", {className: "txt_ct feed-in-group", style: {'marginBottom':'0', 'clear':'both', 'marginTop':'10px'}}, 
      							React.createElement("li", {style: {'margin':'2px 5px 0 0'}}, React.createElement("img", {src: "../images/icon_division.gif", width: "6", height: "14"})), 
        						React.createElement("li", {ref: "feedInGroupFeed", rel: rel, style: {'cursor':'pointer'}, onClick: this.goGroupContents}, React.createElement("strong", null, followerName))
      						)
	           		);
				} else {
	        		return (
		...
		var NotiMessage = React.createClass({displayName: "NotiMessage",
			...
			render: function() {
				...
				if(selectedTab == 'PERSON_TOTAL' || selectedTab == 'PERSON') {
					if(this.state.noticeCnt == 0 || this.props.memberId != _Feed_session_memberId) {
			    		return (
			    				React.createElement("div", {id: "no_message"})
						);
					} else {
						return (
			    				React.createElement("div", {id: "noti_message", onClick: this.showNewFeed}, this.state.noticeCnt, _FEED_Feed_MSG_MESSAGECNTMSG)
						);
					}
				} else if(selectedTab == 'GROUP_TOTAL') {
		...
		var FeedSearchSortOption = React.createClass({displayName: "FeedSearchSortOption",
			getInitialState: function () {
	       		return {};
	    	},
	    	componentDidMount : function() {
			...
			searchStart : function(fromdate , todate, i) {
				...
				React.render(React.createElement(FeedBox, {dateObj: dateObj, baseurl: _Feed_BASE_FEED, memberId: _Feed_session_memberId, groupId: 0}), document.getElementById('MemberFeedBox'));
			},
			componentWillUnmount: function() {
			...
			render : function() {
				return (
					React.createElement("div", null, 
						React.createElement("p", {className: "title"}, "등록일기준"), 
						React.createElement("div", {className: "date-select"}, 
							React.createElement("button", {className: "selected"}, "전체"), 
							React.createElement("button", null, "1주"), 
							React.createElement("button", null, "1개월"), 
							React.createElement("button", null, "1년")
						), 
						React.createElement("p", {className: "title2"}, "직접입력"), 
						React.createElement("div", {className: "data-insert"}, 
							React.createElement("input", {type: "text", style: {'width':'120px'}}), 
		...
		// 피드 박스 DIV
		var FeedBox = React.createClass({displayName: "FeedBox",
			...
			notify: function(data) {
				if(this.state.clickNoti = 'true') {
					React.render(React.createElement(NotiMessage, {showNewFeed: this.showNewFeed, initial: 'false', groupId: this.props.groupId, memberId: this.props.memberId}), document.getElementById('feed_noti'));
			...
			componentDidMount: function() {
				...
				this.getFeedList();
				React.render(React.createElement(NotiMessage, {showNewFeed: this.showNewFeed, initial: 'true', groupId: this.props.groupId, memberId: this.props.memberId}), document.getElementById('feed_noti'));
				var self = this;
				...
				if(document.getElementById('_feedSearchSortOption')) {
					React.render(React.createElement(FeedSearchSortOption, {dateObj: this.props.dateObj}), document.getElementById('_feedSearchSortOption'));
				}
			...
			showNewFeed: function(data) {
				...
				React.render(React.createElement(FeedList, {data: data, fromNoti: 'Y', groupId: this.props.groupId, groupInfo: this.props.groupInfo}), document.getElementById(element.id));
			...
			feedRender:function(data){
				...
				if( data === undefined || data == null || data.length == 0) {
					...
					if(data.length == 0) {
						...
						if(divIdNum == 1)
							React.render(React.createElement(NoFeedHere, null), document.getElementById('noMoreFeedHere'));
						else
					...
					if(this.props.option === undefined) {
						React.render(React.createElement(FeedList, {data: data, groupId: this.props.groupId, groupInfo: this.props.groupInfo}), document.getElementById(curMoreDivId));
			...
			render: function() {
		    	...
		    	return (
		    		React.createElement("div", null, 
		    			React.createElement("span", {className: "top_info_text"}, 
						...
						React.createElement("span", {onClick: this.getFeedList, className: "btn_feedmore", style: {'marginTop':'50px'}}, _FEED_Feed_MSG_MORETEXT), 
						React.createElement("span", {className: "img_feedmore_", style: {'display':'none'}}, React.createElement("img", {src: "../images/loader_32.GIF"}))
					)
				);
			}
		});
		...
		function getMemberInfo(fromTargetMemberId, fromTargetMemberName, option){
			...
			// 가운데 컨텐츠 상단 부분
			React.unmountComponentAtNode(document.getElementById('head_contents'));
			React.render(React.createElement(MemberHead, {memberId: fromTargetMemberId}), document.getElementById('head_contents'));
			...
			if(_Feed_session_memberId == fromTargetMemberId){
				...
				React.render(React.createElement(FeedApp, {targetId: fromTargetMemberId, targetName: fromTargetMemberName}), document.getElementById('mainContentsArea'));
			}

			// 중하단 탭 리스트 및 피드
			React.unmountComponentAtNode(document.getElementById('selectTabBySession'));
			if(_Feed_session_memberId == fromTargetMemberId){
				React.render(React.createElement(MemberTabList, {memberId: fromTargetMemberId, option: option}),  document.getElementById('selectTabBySession'));
			}else{
				React.render(React.createElement(OtherMemberTabList, {memberId: fromTargetMemberId}),  document.getElementById('selectTabBySession'));
			}
		}
		...
		function answerTag(feedId, regMember, removeAnswerTag){
			...
			React.render( React.createElement(Answer, {receiveMember: regMember, removeAnswerTag: removeAnswerTag}), document.getElementById(answerTagId) );
			var cfId = 'cf' + feedId;
			$('#'+cfId).show();
		...
		function feedOGTrigger(feedId, data) {
			...
			React.render(React.createElement(MainOG, {data: data, showRemove: showRemove}), document.getElementById(ogAreaId));
		}
		...
		function feedShare(shareFollower, memberId, feedId) {
			React.render(React.createElement(SharePopup, {shareData: shareFollower, shareMemberId: memberId, shareFeedId: feedId}), document.getElementById('share_popup'));
		...
		function feedChangeFeedDate(reactElement, dateText) {
			...
			if(reactElement.state.dueDate !== undefined && reactElement.state.dueDate.length>0) {
				...
				React.render(React.createElement(ChangeDueDate, {feedId: reactElement.state.feedId, dateText: dateText}), document.getElementById(duedateAreaId));
		...
		function feedAjaxCallBackNoti(reactElement, data) {
			...
			if(cnt > 0) {
				...
				React.render(React.createElement(CommentNoti, {initialDttm: loadDttm, newCommentShow: reactElement.newCommentShow, noticeCnt: cnt}), document.getElementById(notiMsgId));
			}
		}
		...
		function feedMakeGroup(reactElement) {
			...
			React.render(React.createElement(FollowerApp, {fromFollowerInputSetting: reactElement.fromFollowerInputSetting, writtenFollowerList: feedFromFollowerList, fromFollowrSetting: reactElement.fromFollowrSetting, followerHandler: reactElement.followerHandler, ftype: 'normal'}), document.getElementById('_gflist'));
		...
		function feedLikeitResult(reactElement, data) {
			...
			if(reactElement.state.likeItByMe === 1) {
				...
				React.unmountComponentAtNode(document.getElementById(feedLikeItByMeAreaId));
				React.render(React.createElement(LikeIt, null), document.getElementById(feedLikeItByMeAreaId));
				likeItByMe = 1;
			}
    	...
    	function feedComponentDidMount(reactElement) {
			...
			if(reactElement.state.feedType === 'APPROVAL' && reactElement.props.approvalStatus === undefined) {
				...
				React.render(React.createElement(ApprovalComment, {taskId: reactElement.props.taskId, completeAppr: reactElement.completeAppr, feedId: reactElement.state.feedId}),document.getElementById(apprCommentId));
			}
			...
			if($("#"+sliderId).length == 1)
				React.render(React.createElement(Slider, {fileList: reactElement.props.files, feedtype: 'feed_', feedId: reactElement.state.feedId}), document.getElementById(sliderId));
			...
			if(reactElement.state.dueDate !== undefined && reactElement.state.dueDate.length>0) {
				...
				if($("#" + duedateAreaId).length == 1)
					React.render(React.createElement(ChangeDueDate, {option: 'mount', feedId: reactElement.state.feedId, dateText: dueDate}), document.getElementById(duedateAreaId));
			}
		...
		var FanaticElem = React.createClass({displayName: "FanaticElem",
		...
		function showFanaticList(feedId, isCmt) {
			...
			ajaxGet(baseurl, {}, function(data){
				React.render(React.createElement(FanaticElem, {data: data}), document.getElementById('fanaticlist' + feedId));
