		...
		var SharepointDoc  = React.createClass({displayName: "SharepointDoc",
			...
			goSharepointDocList: function() {
				...
				if (mainHeadInfoVal == "group" && path == "FILE") {
					...
					if (siteId == "" && shpFileRepoId == "") {
						...
						return;
//						url = contextpath + "/openapi/sharepoint/site/info?UserID=" + _SharepointDocList_USERID + "&SiteType=" + _SharepointDocList_SITETYPE + "&path=" + path;
					} else {
						url = contextpath + "/openapi/sharepoint/folder/info/" + path + "?UserID=" + _SharepointDocList_USERID + "&SiteID=" + siteId + "&mainType=group&FolderId=" + shpFileRepoId;
					}
...
//					}
				...
				$("#tree").dynatree({
					...
					onLazyRead: function(node){
						...
						var lazyUrl = contextpath + "/openapi/sharepoint/folder/info/" + path + "?UserID=" + _SharepointDocList_USERID + "&SiteID=" + siteId + "&FolderId=" + folderId;
						node.appendAjax({
							url: lazyUrl,
							success: function(nd) {
								checkTreeNode(node);
		...
		function openSharepointDocPop(shpInputId, path, fileHiddenId){
			...
			React.render(React.createElement(SharepointDoc, {path: path}), document.getElementById('SharepointDocPopup'));
