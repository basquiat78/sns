...
(function ($, _, undefined) {
    ...
    var MentionsInput = function (settings) {
        ...
        function initTextarea() {
            ...
            elmInputBox.bind('keydown', onInputBoxKeyUp); //Bind the keydown event to the text area
        ...
        function addMention(mention) {
            ...
            var updatedMessageText = start + mention.value + ' ' + end.substring(1);
        ...
        function onInputBoxInput(e) {
            ...
            var searchBuffer = inputBuffer;
            if(insertBuffer.length > 0) searchBuffer = insertBuffer;
            var triggerCharIndex = _.lastIndexOf(searchBuffer, settings.triggerChar); //Returns the last match of the triggerChar in the inputBuffer
            
            if (triggerCharIndex > -1) { //If the triggerChar is present in the inputBuffer array
                currentDataQuery = searchBuffer.slice(triggerCharIndex + 1).join(''); //Gets the currentDataQuery
                currentDataQuery = utils.rtrim(currentDataQuery); //Deletes the whitespaces
                _.defer(_.bind(doSearch, this, currentDataQuery)); //Invoking the function doSearch ( Bind the function to this)
                if(insertBuffer.length > 0) insertBuffer = [];
            }
        }
        ...
        function onInputBoxKeyUp(e) {
        	...
        	if(e.keyCode != KEY.DOWN) {
	        	var textAreaValue = elmInputBox.val();
	        	var match = (textAreaValue).match(new RegExp(settings.triggerChar + '\\S+', "g"));
	            if(match != null) {
	            	var regexp = /[a-z0-9]|[ \[\]{}()<>?|`~!@#$%^&*-_+=,.;:\"'\\]/g;
	            	var matchValue = match[0];
	            	matchValue = matchValue.substring(1, matchValue.length+1);
	            	if(!regexp.test(matchValue)) {
	            		insertBuffer = [];
	            		insertBuffer.push(settings.triggerChar);
	            		for(var i=0; i<matchValue.length; i++) {
	            			insertBuffer.push(matchValue.substring(i, i+1));
	            		}
	            	}
	            }
        	}
        ...
        function populateDropdown(query, results) {
            ...
            _.each(results, function (item, index) {
                ...
                elmListItem = elmListItem.appendTo(elmDropDownList.css('z-index', '999999')); //Insets the elmListItem to elmDropDownList
            });
	    ...
	    function positionAutocomplete(elmAutocompleteList, elmInputBox) {
            ...
            if (elmAutocompleteListPosition == 'absolute') {
                ...
                elmAutocompleteList.css('z-index', '999999');
